...
- **이 레포는 PR도, 작업 브랜치도, 워크트리도 쓰지 않는다.** 별도 브랜치나 git worktree를 만들지 말고 `main` 체크아웃에서 직접 작업·커밋한다. (전역 브랜치/워크트리 원칙보다 이 레포 규칙이 우선이다.) 백그라운드 작업 등 하네스가 강제로 worktree를 격리시킨 경우에도, 변경사항은 원래 `main` 체크아웃으로 옮겨 커밋하고 격리된 worktree는 작업 완료 후 정리한다.
