...
description: >
  ...
  scope는 변경된 파일 경로로 자동 감지한다 (plugins/common → common, plugins/development → development).
...
| 변경 파일 경로          | scope      |
|------------------------|------------|
| `plugins/<name>/` 하위  | `<name>` (common·design·development 등 — **디렉터리명이 곧 scope**, 하드코딩하지 않는다) |
...
- 이 스킬은 커밋까지만 담당한다. push 여부는 이 스킬의 관심사가 아니며, 호출한 세션의 CLAUDE.md 규칙을 따른다
