...
## [common/0.26.0] — 2026-08-31

### ♻️ Refactor
- `retrospective`: 회고 대상을 **스킬 문서와 상시 로드 지침으로만** 좁혔다. 사용자가 암묵적 지침보다 명시적 지침 문서를 선호해, 메모리 카테고리(적격성 검증 게이트 + `memory-curator` 자동 전달), 볼트 회고 로그(회사 `docs/records/retrospective/`·개인 볼트 `retrospective/YYYY-MM-DD.md` + "문서 저장 형식" 섹션), 문서화 공백 추천을 제거했다. 남은 3개 카테고리(스킬 개선 / 신규 스킬 추천 / 상시 로드 지침 계층)는 전부 사용자 확인 후 반영으로 통일했다 — 메모리 자동 반영 분기가 사라지며 Step 4가 단순해졌다. 다중 세션·주간 회고 워크플로우는 유지(반복 패턴 → 상시 로드 지침 후보)

---

## [common/0.18.2] — 2026-07-21

### ♻️ Refactor
- `draft-plan`: 계획서 템플릿의 "스파이크"(애자일 음차 용어)를 "선행 검증"으로 교체했다. 팀원에게 생소한 용어라, `## 기술 리스크 / 스파이크` 섹션 제목·본문 6곳(조사 포함)을 평이한 한국어로 순화했다 — 개념 설명(버리는 최소 코드로 미검증 전제를 찌르는 커밋 0 게이트)은 그대로 유지 ([`e03e2cd`])
...
### ✨ New Features
- `design-md`: 그린필드거나 사람이 육안검증·공유가 필요할 때 자기완결 `preview.html`(컬러 스와치·타입 스케일·스페이싱 바·컴포넌트 갤러리 + 라이트/다크 토글, 외부 참조 0)을 조건부로 함께 생성한다. DESIGN.md에 다크 팔레트가 있으면 토글이 브랜드까지 스왑하고, 없으면 페이지 배경만 전환해 없는 다크 색을 지어내지 않는다("현실 기술" 정체성 유지). 함께 Step 5의 그린필드 참조-앵커 누락(CLAUDE.md·AGENTS.md가 둘 다 없으면 DESIGN.md가 앵커 없이 남아 자동 로드가 안 되던 문제)을 AGENTS.md 생성 제안으로 수정 ([`77985da`])
...
### ✨ New Features
- `notion-write` 스킬 추가 — 노션 MCP로 페이지를 만들 때 콜아웃·토글·컬럼·표·목차 같은 네이티브 블록을 "언제 어디에" 쓸지 규칙화해 텍스트 벽을 방지하는 스타일 가이드. 진입부(아이콘·요약 콜아웃·목차)→본문 골격→블록 선택→강조 규율→생성 후 검증 5단계 워크플로우, 블록 선택 치트시트와 AI 안티패턴, 문서 유형별(회의록·가이드·위키·보고서) 레이아웃 레시피를 references로 자산화 ([`29dc011`])

### ♻️ Refactoring
- `retrospective`·`skill-metrics` 문서 저장 위치 표기를 "볼트"에서 "문서"로 통일하고, 경로는 공개 레포에 맞게 플레이스홀더로 일반화. `retrospective` 회고 출력을 서술 섹션 없이 "확인이 필요한 업데이트 / 메모리(자율 반영)" 목록 중심으로 개편 — 확인 항목이 없으면 섹션을 생략 ([`dbfa200`])
- `review-pack` 스킬 제거 ([`d91ee2d`])
