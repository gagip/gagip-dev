...
### 🐛 Fix
- **commit**: CI 주간 eval `commit-basic`이 3회 중 1회 실패하던 원인을 고쳤다. 스크래치 메모 파일을 커밋에 넣을지 사용자에게 되물어 커밋을 만들지 못하는 경우가 있었다. 어떤 파일을 포함할지 되묻지 않고 스테이징 여부만으로 판단해 바로 진행한다는 원칙을 행동 원칙에 명시했다 (`8f9172d`)
