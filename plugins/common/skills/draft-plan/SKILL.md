...
초안(목표·추적 매트릭스·진행 현황·로그를 포함한 본문)이 완성되면 **승인을 기다리지 않고 곧바로** `notion-report` 스킬을 호출해 Notion Reports DB에 저장한다. 사용자는 노션에서 읽고 **댓글로 피드백**한다 — 그 편이 사용자에게 더 빠르고, 화면 승인 클릭을 없앤다.
...
- 유형: `구현계획`
- 제목: 계획서 제목
- 본문: 작성한 계획서 전문 (형식 그대로 — 표·콜아웃은 마크다운 그대로 전달한다. notion-report가 Notion 렌더 문법(`<table>`/`<callout>`)으로 변환해 저장한다)
