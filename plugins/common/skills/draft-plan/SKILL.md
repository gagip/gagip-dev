...
1. **빌트인 — 항상 `medium` 으로 명시**
   ...
   ⚠️ **일부 모델·환경에서는 `/code-review`가 Skill 도구로 호출되지 않을 수 있다** ("cannot be used with Skill tool due to disable-model-invocation" 에러). 비용 관리를 위해 모델별로 모델발(model-invoked) 호출을 막아 두었을 가능성이 있으나 확정된 사실은 아니다. 이 에러가 나면 재시도하거나 우회를 찾지 말고, 사용자에게 `/code-review medium`을 직접 실행해 달라고 요청하거나(터미널에 슬래시 커맨드로 직접 입력) 이번 변경엔 생략해도 되는지 확인한다.
