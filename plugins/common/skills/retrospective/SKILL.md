...
직접 띄운 에이전트와 **워크플로우 스크립트가 띄운 에이전트를 함께** 낸다. 뒤쪽은 대화에 도구 호출로
남지 않아(부모가 부른 것은 워크플로우 하나뿐이다) 따로 읽지 않으면 작업 전체를 워크플로우로 돌린
회고에서 위임 내역이 통째로 "실행 없음"이 된다. 워크플로우 단계에는 직후 사용자 발화를 짝짓지
않는다 — 사용자는 회차 전체를 기다렸지 단계 하나하나에 반응하지 않았고, 짝지으면 같은 발화가 모든
단계의 "반응"으로 복제된다.
...
- [ ] [지침·전역] 문서 고칠 때 그림 안 글자도 함께 보기 (왜: …)
...
- 각 항목엔 **왜**를 한 줄로 붙인다.
- 항목 이름은 고칠 파일의 절 이름이 아니라 **다음에 무엇이 달라지는지**로 쓴다 (예: "◯◯ 원칙에 항목 추가" ✗ → "문서 고칠 때 그림 안 글자도 함께 보기" ○).
