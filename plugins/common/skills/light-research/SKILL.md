...
allowed-tools: Workflow, Read, Write, Edit, WebSearch, WebFetch, Agent
...
현재 `SKILL.md` 디렉터리의 `references/light-research-template.js`를 읽는다.

- `Workflow` 도구가 있으면 기존 동작을 보존한다. 템플릿 구조를 그대로 따라 인라인 스크립트로
  전달하고, `ANGLES` 배열과 synthesis 프롬프트 출력 구조만 바꾼다.
- `Workflow`가 없고 서브에이전트 위임이 가능하면 탐색 각도별 독립 작업을 병렬 배분하고, 결과가
  모두 모인 뒤 한 번만 종합한다.
- 둘 다 없으면 현재 제공된 웹 검색·페이지 읽기 도구로 같은 탐색 각도를 순차 조사한 뒤 직접
  한 번 종합한다. 실행 방식만 달라지고 각도 수·출처 기준·출력 형식은 바꾸지 않는다.
...
저장을 원하면 현재 하네스가 읽는 프로젝트 지침 파일(`AGENTS.md`, `CLAUDE.md` 등)의 볼트 규칙
(frontmatter 필수, 노트 하나 = 개념 하나, 관련 `_index.md`에 링크 추가)을 따라 `wiki/concepts/`
또는 `wiki/topics/<주제>/`에 저장한다. 이미 관련 노트가 있다면 중복 노트를 새로 만들지 말고,
기존 노트를 갱신하거나 별도 심화 노트를 만들어 링크로 연결할지 사용자와 상의한다.
