...
**Base URL·인증 헤더 형식·엔드포인트 경로 모양은 확인 대상이 아니다.** 이런 API 형태 정보가
없다고 되묻느라 파일을 하나도 못 만드는 것보다, REST GET + `Authorization: Bearer <token>` 같은
합리적 기본값으로 일단 서버를 만들고 완료 보고서에 "이 부분은 추정했다, 실제 형태가 다르면
`PATH_TEMPLATE`/`render()` 등 이 부분만 고치면 된다"고 남기는 쪽이 낫다.
