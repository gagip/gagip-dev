...
참조된다. 이걸 빠뜨리면 파일이 있어도 에이전트가 무시한다. 대상 파일(있으면 `CLAUDE.md`, 없으면 `AGENTS.md`, 둘 다 없으면
안내만)에 아래 줄 추가를 **제안하고, 사용자가 동의하면 `Edit`로 추가**한다:
...
### Step 6: 확장 안내 (v2 — 생성하지 않음)

이 스킬 v1은 **DESIGN.md 단일 파일**만 생성한다. 아래 부수 산출물은 **조건부로 유용하지만 지금은 만들지 않고, 필요 조건이
보이면 사용자에게 옵션으로만 안내**한다 (템플릿에도 주석 슬롯으로 표시돼 있다):

- **`preview.html`(+dark)** — 컬러 스와치·타입 스케일·컴포넌트 갤러리를 한 화면에 렌더. **렌더되는 실앱이 없거나
  (그린필드) 사람이 온-브랜드 여부를 육안 검증/공유해야 할 때** 유용. 이미 돌아가는 앱이 있으면 중복이다.
- **토큰 파일** — 빌드 파이프라인(Tailwind/Style Dictionary 등)이 감지되면 DTCG JSON 정본 + CSS 변수(`--ds-*`) 소비판으로
  분리 emit이 유용.

> 무거운 부수 세트를 항상 만들지 않는 이유: 성숙 프로덕션에서 DESIGN.md에 모든 걸 인라인하면 토큰을 크게 과소비하고,
> 에이전트가 기존 컴포넌트를 재사용하지 않고 재생성하는 기술부채가 생긴다는 실측 보고가 있다. 그래서 코어는 가볍게,
> 부수는 조건부로 둔다.
...
- **DESIGN.md 하나만 생성한다.** v1은 preview.html·토큰 파일을 만들지 않는다(조건 보이면 안내만). 코드(컴포넌트 등)를
  수정하지 않는다 — `Write`는 DESIGN.md에, `Edit`는 CLAUDE.md/AGENTS.md 참조 줄 추가에만 쓴다.
