...
<!-- ══════════════ v2 확장 슬롯 (현재 미생성) ══════════════
 아래 부수 산출물은 조건이 맞을 때만 별도로 만든다. 지금은 생성하지 않는다.
   preview.html (+preview-dark.html) — 스와치·타입스케일·컴포넌트 갤러리.
       → 렌더되는 실앱이 없거나(그린필드) 사람이 온-브랜드 육안검증/공유가 필요할 때.
   tokens.json — DTCG 정본 ($value/$type). tokens.css — --ds-* 소비판.
       → Tailwind/Style Dictionary 등 빌드 파이프라인이 있을 때.
 무거운 부수 세트를 항상 만들지 않는 이유: 성숙 프로덕션에선 인라인 과다가 토큰 과소비·컴포넌트 재생성 부채를 부른다.
═══════════════════════════════════════════════════════ -->
