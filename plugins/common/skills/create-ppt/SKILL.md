...
description: >
  ...
  에이전트가 HTML을 직접 쓰지 않고 번들된 Python 스크립트에 위임하므로
...
에이전트는 내용을 JSON으로 수집하고 스크립트를 실행하기만 한다 — HTML 작성은 전혀 없다.

슬라이드는 **컴포넌트 조립** 방식으로 동작한다:
- JSON의 `slides` 배열로 어떤 슬라이드를 어떤 순서로 넣을지 에이전트가 결정한다.
...
구조화 질문 도구가 있으면 핵심 정보를 한 번에 묻는다. 없으면 같은 항목을 번호 목록으로
대화에서 묻는다.
...
현재 `SKILL.md`가 들어 있는 디렉터리를 절대 경로로 해석해 그 아래 `scripts/generate_ppt.py`를 실행한다.

```bash
# 출력 파일명: 프로젝트명_브리핑.html (또는 사용자 지정)
OUTPUT_FILE="${PROJECT_NAME}_브리핑.html"

python3 "<스킬 디렉터리>/scripts/generate_ppt.py" \
