...
| `changelog`    | "체인지로그 써줘", "릴리즈 노트 만들어줘"       |
| `validate-skill` | Claude 자동 참조 (사용자 직접 호출 불가)      |
...
Git 커밋 이력을 분석해 CHANGELOG 초안을 작성하고, 승인 후 `CHANGELOG.md`에 저장한다. 태그 범위를 자동 감지하며, 직접 범위를 지정할 수도 있다.
...
새 스킬을 작성하거나 기존 스킬을 수정한 직후 Claude가 자동으로 참조하는 품질 검증 스킬. 필수 frontmatter 필드, `[STOP]` 패턴, `allowed-tools` 일관성을 점검한다. 사용자가 직접 호출할 수 없다 (`user-invocable: false`).
